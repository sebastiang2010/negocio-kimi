...
2. `BotApp` - 18 edges
3. `EventLogger` - 15 edges
4. `Update` - 13 edges
5. `DEFAULT_TYPE` - 12 edges
6. `PLAYBOOK v0.1 — Procedimientos operativos` - 11 edges
7. `Announcements` - 10 edges
8. `FaqHandler` - 10 edges
9. `nullforge — Telegram Moderator + FAQ Bot` - 10 edges
10. `TestModeration` - 9 edges
...
- `BotApp` --uses--> `FaqHandler`  [INFERRED]
  productos/bot-telegram/src/bot.py → productos/bot-telegram/src/faq.py
- `BotApp` --uses--> `EventLogger`  [INFERRED]
  productos/bot-telegram/src/bot.py → productos/bot-telegram/src/log.py
...
Cohesion: 0.22
Nodes (4): FaqHandler, match_faq(), FAQ: matching por keywords + fallback opcional a LLM local (Ollama)., TestFaq
...
- **Are the 9 inferred relationships involving `EventLogger` (e.g. with `BotApp` and `Update`) actually correct?**
  _`EventLogger` has 9 INFERRED edges - model-reasoned connections that need verification._
